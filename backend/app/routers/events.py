...
#
# Keep these in sync with FLYER_TEMPLATES/FLYER_BACKGROUNDS/FLYER_FONTS in
# frontend/src/flyerTemplates.js.
...
# Derived, not hand-typed — a template_id added to this set without a
# matching FLYER_TEMPLATE_DEFAULTS entry would pass the membership check
# below and then KeyError on the defaults lookup right after it.
FLYER_TEMPLATE_IDS = frozenset(FLYER_TEMPLATE_DEFAULTS)
...
async def upload_event_flyer(event_id: int, file: UploadFile, request: Request, db: Session = Depends(get_db)):
    ...
    tags_by_event = _tags_by_event(db, [event_id], viewer_id=event.host_id)
